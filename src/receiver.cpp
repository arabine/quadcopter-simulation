...
receiver::receiver(IKeyboard &kb)
	: keyboard(kb)
{
...
void receiver::get_desired_theta(Vector3d &theta_d)
{
	...
	{
		// roll (A and D)
		if(keyboard.keypressed(IKeyboard::KEY_A))
			theta_d(0) -= this->roll_pwm;
		else if(keyboard.keypressed(IKeyboard::KEY_D))
			theta_d(0) += this->roll_pwm;

		// pitch (W and S)
		if(keyboard.keypressed(IKeyboard::KEY_W))
			theta_d(1) += this->pitch_pwm;
		else if(keyboard.keypressed(IKeyboard::KEY_S))
			theta_d(1) -= this->pitch_pwm;

		// yaw (Q and E)
		if(keyboard.keypressed(IKeyboard::KEY_Q))
			theta_d(2) -= this->yaw_pwm;
		else if(keyboard.keypressed(IKeyboard::KEY_E))
			theta_d(2) += this->yaw_pwm;
	}
...
{	
	...
	{
		// climb (+)
		if(keyboard.keypressed(IKeyboard::KEY_PLUS))
			throttle += this->throttle_pwm;
		// sink (-)
		else if(keyboard.keypressed(IKeyboard::KEY_MINUS))
			throttle -= this->throttle_pwm;
	}
